    ...
    public class FileHelperManager : IFileHelper
    {
        ...
        public string Upload(IFormFile file, string root)
        {
            if (file.Length > 0)//file.Length=>Dosya uzunluğunu bayt olarak alır. burada Dosya gönderilmiş mi gönderilmemiş diye test işlemi yapıldı.
            {
                ...
                {
                    file.CopyTo(fileStream);//Kopyalanacak dosyanın kopyalanacağı akışı belirtti. yani yukarıda gelen IFormFile türündeki file dosyasını nereye kopyalacağını söyledik.
